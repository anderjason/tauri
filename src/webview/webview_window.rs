...
impl<'a, R: Runtime, M: Manager<R>> WebviewWindowBuilder<'a, R, M> {
    ...
    pub fn on_menu_event<
        F: Fn(&crate::Window<R>, crate::menu::MenuEvent) + Send + Sync + 'static,
    >(
        mut self,
        f: F,
    ) -> Self {
...
impl<'a, R: Runtime, M: Manager<R>> WebviewWindowBuilder<'a, R, M> {
    ...
    pub fn traffic_light_position<P: Into<Position>>(mut self, position: P) -> Self {
        ...
        // `WebviewWindow` may be hosted by the native window view hierarchy instead
        // of a Wry-owned content view, so both builders must receive the same inset.
