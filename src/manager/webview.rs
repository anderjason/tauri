...
use super::{
    window::{
        DragDropPayload, DRAG_DROP_EVENT, DRAG_ENTER_EVENT, DRAG_LEAVE_EVENT, DRAG_OVER_EVENT,
    },
...
pub struct UriSchemeProtocol<R: Runtime> {
    ...
    pub handler: Box<
        dyn Fn(UriSchemeContext<'_, R>, http::Request<Vec<u8>>, UriSchemeResponder) + Send + Sync,
    >,
}
    ...
    fn prepare_pending_webview<M: Manager<R>>(
      ...
      "
        ...
        all_initialization_scripts
            .push(main_frame_script(self.invoke_initialization_script.clone()));
    ...
    pub fn prepare_webview<M: Manager<R>>(
        ...
        let mut url = match &pending.webview_attributes.url {
            ...
            WebviewUrl::External(url) => {
                let config_url =
                    app_manager.get_app_url(pending.webview_attributes.use_https_scheme);
    ...
    pub(crate) fn attach_webview(
        ...
        #[cfg(all(target_os = "ios", feature = "wry"))]
        {
            webview
                .with_webview(|w| {
                    unsafe {
                        crate::ios::on_webview_created(w.inner() as _, w.view_controller() as _)
                    };
                })
