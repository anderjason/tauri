        ...
        fn filter_target(target: &EventTarget, candidate: &EventTarget) -> bool {
            match target {
                ...
                EventTarget::Window { label } => match candidate {
                    EventTarget::AnyLabel { label: l } | EventTarget::Window { label: l } => {
                        l == label
                    }
                    _ => false,
                },
                EventTarget::Webview { label } => match candidate {
                    EventTarget::AnyLabel { label: l } | EventTarget::Webview { label: l } => {
                        l == label
                    }
                    _ => false,
                },
                EventTarget::WebviewWindow { label } => match candidate {
                    EventTarget::AnyLabel { label: l }
...
mod test {
    ...
    fn setup_events(setup_any: bool) -> EventSetup {
        ...
        let webview_window =
            WebviewWindowBuilder::new(&app, "main-webview-window", Default::default())
                .build()
                .unwrap();
        ...
        macro_rules! setup_listener {
            ($type:ident, $id:ident, $any_id:ident) => {
                ...
                $type.listen(TEST_EVENT_NAME, move |evt| {
                    tx_.send(($id, serde_json::from_str::<String>(evt.payload()).unwrap()))
                        .unwrap();
                });
