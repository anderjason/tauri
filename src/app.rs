...
pub type ChannelInterceptor<R> = Box<
    dyn Fn(&Webview<R>, CallbackFn, usize, &InvokeResponseBody) -> bool + Send + Sync + 'static,
>;
...
impl<R: Runtime> AppHandle<R> {
    ...
    pub async fn fetch_data_store_identifiers(&self) -> crate::Result<Vec<[u8; 16]>> {
        let (tx, rx) =
            tokio::sync::oneshot::channel::<Result<Vec<[u8; 16]>, tauri_runtime::Error>>();
    ...
    ($app: ty) => {
        impl<R: Runtime> $app {
            ...
            pub fn on_tray_icon_event<
                F: Fn(&AppHandle<R>, TrayIconEvent) + Send + Sync + 'static,
            >(
                &self,
                handler: F,
            ) {
            ...
            pub fn monitor_from_point(&self, x: f64, y: f64) -> crate::Result<Option<Monitor>> {
                Ok(match self.runtime() {
                    ...
                    RuntimeOrDispatch::RuntimeHandle(h) => {
                        h.monitor_from_point(x, y).map(Into::into)
                    }
                    _ => unreachable!(),
                })
            }
...
```
  "####
    ...
    where
        F: FnOnce(&mut App<R>) -> std::result::Result<(), Box<dyn std::error::Error>>
            + Send
            + 'static,
    {
            ...
            msg_hook: {
                ...
                Some(Box::new(move |msg| {
                    use windows::Win32::UI::WindowsAndMessaging::{
                        TranslateAcceleratorW, HACCEL, MSG,
                    };
                    unsafe {
        ...
        RuntimeRunEvent::Ready => {
            ...
            #[cfg(all(dev, target_os = "macos"))]
            {
                ...
                if let Some(icon) = app_handle.manager.app_icon.clone() {
                    ...
                    let app_icon =
                        NSImage::initWithData(NSImage::alloc(), &data).expect("creating icon");
