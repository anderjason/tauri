...
fn run() -> Result<(), Box<dyn std::error::Error>> {
  ...
  use tauri::{
    ActivationPolicy, Builder, LogicalPosition, LogicalSize, Manager, TitleBarStyle, WebviewUrl,
    Wry,
  };
  ...
  let window = tauri::WebviewWindowBuilder::new(&app, "traffic-lights", WebviewUrl::default())
    ...
    // Keep this offset obviously non-default so propagation or layout
    // regressions are immediately visible in both the assertions and the test.
